...
0 1 * * * cd <redminedir>; rake redmine:check_periodictasks RAILS_ENV="production"

on each project it will add a new tab named Scheduled Tasks just go there to add your task

(note: this is fork of original plugin with missing translations and additional features)
